...

{
	...
	*/
	...
	ask_opts ();            /*      get user options                */
	puts("TI-99 filenames cannot contain a '.', so '_' is");
	puts("typically used to separate a filename extension.");
	puts("Listing *_c:");
	system("ls *_c");
	input = getfn ("Input", "r");   /*      Get initial input file  */
...
{
	puts("Common practice is to replace '_c' in the source filename");
...
** (i.e. define for use)   */
/*
** adds symbol table entry with appropriate
...
*/
/* return subscript value if found
...
/*                              putint (argtop - getint (argptr + OFFSET), argptr + OFFSET); */
