 ...
 * or if the supplied ptr is * zero, will call the address in R8
 */
...
/*
 * search global symbol table * return cptr if name found * return 0 if not
 * found (cptr->new entry) * set cptr2=0 if table full
 */
...

{
    ...
    {
//FIXME:Always false in standard C.Does this work on
	    // the TI when compiling with C99 ?
	    if (item == 16384 << 1)
	{
	    outstr("32768");
	    return (5);
 ...
 *
 * * lval [0]-symbol table address, 0 for constant * lval [1]-type of
 * indirect object to fetch, 0 for static * lval [2]-type of pointer or
 * array, 0 otherwise * lval [3]-Table Number: *      0 = Unused. *      1 =
 * Global Symbol-Table. *      2 = Local Symbol-Table. *      3 = Macro
 * Symbol-Table.
 */
