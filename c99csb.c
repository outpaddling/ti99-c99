...

{
	...
	while (1)
	{
		...
		unix2dos_path (fn);
	#endif
/*		if (base_dir [0] == (char) 0)
		{
			strcpy (base_dir, dirname (fn));
		} */
#else
		gets (fn);
#endif
...
LITBUF BSS  2
*
* c=getlit(litptr,type); GET LITERAL CHAR OR WORD
*
...
GETL1  B    *13
*
* putlit(c,litptr,type); STORE LITERAL CHAR OR WORD
*
PUTLIT MOV  @2(14),2     #BYTES
       MOV  @4(14),0
       AI   0,LITBAS
       MOV  @6(14),3     VALUE
       LI   1,LITBUF
...
PUTL1  MOV  3,*1
       ...
       B    *13
*
* hash(sname); SYMBOL NAME HASHING
*
* RETURNS EVEN VALUES
*
HASH   MOV  @2(14),0     ADDRESS OF NAME STRING
       CLR  8            VALUE ACCUMULATOR
HASH1  MOVB *0+,1        GET NEXT CHAR
       JEQ  HASHX        IF END OF STRING
       SRL  1,8          TO LSB
       A    1,8          ACCUMULATE
...
*
*
* isspace (c); TRUE IF c IS ALPHABETIC OR _
*
ISSPAC MOV  @2(14),0
...
*
*
* alpha(c); TRUE IF c IS ALPHABETIC OR _
*
ALPHA  MOV  @2(14),0
       CLR  7            CLEAR AN FLAG
       BL   @TST
       B    *13
*
* an(c); TRUE IF c IS ALPHANUMERIC
*
ALNUM  MOV  @2(14),0
       SETO 7            SET AN FLAG
       BL   @TST
       B    *13
*
* numeric(c); TRUE IF c IS NUMERIC
*
NUMERI MOV  @2(14),0
       BL   @TSTN
       B    *13
...
*
* astreq(str1,str2,len); COMPARE str1 AND str2 FOR UP TO
* len CHARACTERS. IF  A MATCH, BOTH MUST TERMINATE WITH
* NON-ALPHA CHARACTERS. RETURNS 0 OR # CHARS MATCHED.
*
...
ASTR3  CLR  8            RETURN FALSE
       B    *13
*
* streq(str); COMPARE str WITH THE INPUT LINE, STARTING
* AT LPTR. IF A MATCH WHEN str's 0-BYTE IS ENCOUNTERED,
* RETURN # CHARS MATCHED. IF NO MATCH, RETURN 0.
*
...
*
* putint(i,addr); STORE i AT addr, LO BYTE FIRST
*
PUTINT MOV  @4(14),1     VALUE
       ...
       B    *13
*
* i=getint(addr); GET i FROM addr, LO BYTE FIRST
*
